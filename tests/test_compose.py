...
def test_update_image_in_compose_data():
    ...
    compose_data = {"services": {"web": {"image": "python:3.9-slim"}}}
...
def test_pin_compose_images_dry_run(mock_get_digest):
    ...
    compose_content = {
        "version": "3.8",
        "services": {"web": {"image": "python:3.9-slim"}},
    }
...
def test_get_image_digest_success(mock_run):
    ...
    mock_result.stdout = json.dumps({"config": {"digest": "sha256:abc123def456"}})
